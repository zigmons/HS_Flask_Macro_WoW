...
ham.Player.new = function()
  ...
  function self.getHealingSpells()
      ...
      if self.englishClass=="EVOKER" then
        if HAMDB.verdantEmbrace then
          if IsSpellKnown(ham.verdantEmbrace) then
            local name, rank, icon, castTime, minRange, maxRange = GetSpellInfo(ham.verdantEmbrace)
            return "24", name
          end
        end
      end
